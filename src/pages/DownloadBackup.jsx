...
import { ReactSelect, Checkbox, MultiInputs, SingleTextInput } from '../component/forms/FormElements'
...
const DownloadImages = () => {
    ...
    // validate form 
    // const validateForm = () => {
    //     validationField('siteName', field.siteName, setError, setErrorMessage)
    //     validationField('customerId', field.customerId, setError, setErrorMessage)
    //     validationField('purpose', field.purpose, setError, setErrorMessage)
    //     validationField('size', field.size, setError, setErrorMessage)
    ...
    //     if ((error.siteName === true || error.customerId === true || error.purpose === true) || error.size || keyword.length <= 0) {
    //         return false
    //     } else {
    //         return true
    //     }
    ...
    // submit form 
    // const submitHandler = () => {
    //     console.log('form submission ', validateForm())
    ...
    const searchKeywordSubmitHandler = (value) => { // keyword  submit handler
        ...
        fetchInventory(1, 12, value, true);
    }
    ...
    const submitHandler = () => {
        ...
        if (keywords && keywords.length > 0) {
            fetchInventory(1, 12, keywords, true);
        } else {
            setShowError(true)
        }
    }
            ...
            function handleScrollForSection() {
                ...
                if (scrollTop + 1 >= scrollHeight - clientHeight) {
                    if (!dataObject.isFetchingData) {
                        ...
                        fetchInventory(dataObject.currentPage, 12, keywords, false);
                    }
